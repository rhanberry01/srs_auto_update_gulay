...
function printHelp(commands, flags) {
    ...
    sortAndGroupCommands_1.sortAndGroupCommands(commands).forEach(({ group, commands: groupCommands }) => {
        ...
        groupCommands.forEach(({ commandName, description }) => {
            console.log(`  ${cliui_1.logger.colors.green(commandName.padEnd(maxWidth, ' '))}  ${cliui_1.logger.colors.dim(description)}`);
        });
    });
    if (flagsList.length) {
        console.log('');
        ...
        flagsList.forEach(({ displayName, displayType, description = '', width }) => {
            ...
            console.log(`  ${cliui_1.logger.colors.green(displayName)} ${cliui_1.logger.colors.dim(displayType)} ${whiteSpace}  ${cliui_1.logger.colors.dim(description)}`);
        });
    }
}
...
function printHelpFor(command) {
    ...
    if (args.length) {
        ...
        args.forEach(({ displayName, description = '', width }) => {
            const whiteSpace = ''.padEnd(maxWidth - width, ' ');
            console.log(`  ${cliui_1.logger.colors.green(displayName)} ${whiteSpace}   ${cliui_1.logger.colors.dim(description)}`);
        });
    }
    if (flags.length) {
        console.log('');
